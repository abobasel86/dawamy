                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="work_shift_id" id="work_shift_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    ...
                                    @foreach($workShifts as $shift)
                                        <option value="{{ $shift->id }}" @selected(old('work_shift_id') == $shift->id)>
                            ...
                            <div>
                                ...
                                <select name="timezone" id="timezone" required class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    @foreach($timezones as $timezone)
                                        <option value="{{ $timezone }}" @selected(old('timezone', 'Asia/Amman') == $timezone)>{{ $timezone }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <!-- Radius -->
                             <div>
                                ...
                                <input id="radius_meters" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="number" name="radius_meters" value="{{ old('radius_meters', 100) }}" required />
                            </div>

                             <!-- Latitude -->
                            <div>
                                <label for="latitude" class="block font-medium text-sm text-gray-700">خط العرض (Latitude)</label>
                                <input id="latitude" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="text" name="latitude" value="{{ old('latitude') }}" required />
                            </div>

                            <!-- Longitude -->
                             <div>
                                <label for="longitude" class="block font-medium text-sm text-gray-700">خط الطول (Longitude)</label>
                                <input id="longitude" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="text" name="longitude" value="{{ old('longitude') }}" required />
                            </div>
